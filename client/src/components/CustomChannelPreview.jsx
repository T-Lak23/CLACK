...
const CustomChannelPreview = ({ channel, setActiveChannel, activeChannel }) => {
  const isActive = activeChannel && activeChannel?.id === channel?.channel?.id;
  const isDM =
    channel?.channel?.data?.member_count === 2 &&
    channel?.channel?.id?.includes("user_");

  if (isDM) return null;

  const unreadCount = channel?.unread_count || 0;
  const channelName =
    channel?.channel?.data?.name || channel?.channel?.id || "Unknown Channel";
